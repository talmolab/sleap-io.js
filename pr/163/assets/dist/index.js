...
async function renderImage(source, options = {}) {
  ...
  const { instances, skeleton, frameSize, frameIdx, tracks, trackIndexMap, lf } = extractSourceData(source, opts);
  if ((opts.overlay === void 0 || opts.overlay === null) && lf && lf.masks.length > 0) {
...
function resolveOverlayTrackColors(scheme, overlay, paletteName, tracks, trackIndexMap) {
  ...
  const nTracks = Math.max(1, tracks.length);
  const palette = getPalette(paletteName, nTracks);
  return overlay.map((el) => {
...
async function renderVideo(source, outputPath, options = {}) {
  ...
  if (videoOverlay === void 0 && !Array.isArray(source)) {
    const targetVideo = selectedFrames[0].video;
    const labels = source;
