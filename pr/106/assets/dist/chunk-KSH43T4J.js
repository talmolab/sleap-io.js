...
async function readVideosStreaming(file, labelsPath, openVideos = false, formatId = 1) {
  try {
    ...
    for (let videoIndex = 0; videoIndex < metadataList.length; videoIndex++) {
        ...
        if (datasetPath) {
          try {
            ...
            if (meta.embedded) {
              ...
              if (framesAttr != null) {
                const val = typeof framesAttr === "object" && framesAttr !== null && "value" in framesAttr ? framesAttr.value : framesAttr;
      ...
      const videoShape = (sourceFrameCount ?? meta.frameCount) && meta.height && meta.width && meta.channels ? [sourceFrameCount ?? meta.frameCount, meta.height, meta.width, meta.channels] : shape;
      let backend = null;
